<?php
	...
	if(isset($_GET["acao"]) && ($_GET["acao"]=="edita")) {
		...
		extract($cliente);
  	    $DataNascimento = strtotime($DataNascimento)!=0 ? date("d/m/Y", strtotime($DataNascimento)) : "";
...
function verificaNascimento(f) {
	var envia = true;
	if(f.DataNascimento.value == "") {
		envia = false;
		alert('Data de nascimento é campo obrigatório.');
	}
	return envia;
}
</script>
...
<form action="<?php if($edita) 
			...
			?>" method="post" onSubmit="return verificaNascimento(this)">
...
<tr>
	...
	<td><input type="text" name="CPF" size="20"
	<?php if($edita) {?>
		value="<?=$CPF?>"
	<?php }?>
	></td>
...
<tr>
	<td class="formulario">Nome do Pai:</td>
	<td><input type="text" name="NomePai" size="20"
	<?php if($edita) {?>
		value="<?=$NomePai?>"
	<?php }?>
	></td>
...
<tr>
	<td class="formulario">Nome da Mãe:</td>
	<td><input type="text" name="NomeMae" size="20"
	<?php if($edita) {?>
		value="<?=$NomeMae?>"
	<?php }?>
	></td>
...
<a href="termosCliente.php?id=<?=$id?>">Ver termos</a>
</body>
</html>