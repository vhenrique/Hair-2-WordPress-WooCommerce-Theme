...
<?php if(comments_open()): ?>
	<div id="respond">
		...
		<?php if(get_option('comment_registration') && !is_user_logged_in()): ?>
			<p>Faça seu <a href="<?php echo wp_login_url(get_permalink()); ?>" title="Fazer login">cadastro ou login</a> gratuito para enviar comentários.</p>
		<?php else : ?>
			<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
				<?php if(is_user_logged_in()): ?>
					<p>Logado como <?php echo $user_identity; ?>. 
					<a href="<?php echo wp_logout_url(get_permalink()); ?>" title="Fazer logout">Logout</a></p>
				<?php else : ?>
